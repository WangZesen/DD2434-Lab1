...
class network:
	...
	def forward(self, inputData):
		...
		for k in range(1, self.config.layer):
			self.values[k] = np.dot(np.array([self.values[k - 1]]), self.w[k]).reshape((self.config.nodes[k],)) + self.bias[k]
	...
	def backward(self, x, y):
		...
		if self.config.mode == 2: # Back Propogation
			for it in range(self.config.maxIter):
				...
				for i in range(self.config.layer):
					self.w[i] = self.w[i] + delta[i] / self.config.batch * (1 - self.config.momentum) + self.lastRoundDelta[i] / self.config.batch * self.config.momentum
					self.bias[i] = self.bias[i] + biasDelta[i] / self.config.batch * (1 - self.config.momentum) + self.lastRoundBiasDelta[i] / self.config.batch * self.config.momentum
	...
	def draw(self):
		...
		for i in range(n):
			for j in range(m):
				...
				'''
		#print (self.w)
		plt.contour(X, Y, Z, [-0.5, 0, 0.5], colors = ['r', 'black', 'b'])
		plt.show()
